...
// Paths & files
$input_path  = implode(DIRECTORY_SEPARATOR, [__DIR__, 'emex']);
$output_path = implode(DIRECTORY_SEPARATOR, [__DIR__, 'markdown']);
$input_files = scandir($input_path);

// Create output directory
if (file_exists($output_path)) die('Error: Delete `markdown` folder and try again');
mkdir($output_path);

// For each exported file...
foreach ($input_files as $file) {
	if (substr($file, -5) === '.enex') {
